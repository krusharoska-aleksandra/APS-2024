...
public class DLLVojska {
    ...
    public static DLL<Integer> vojska(DLL<Integer> lista, int a, int b, int c, int d) {
        ...
        for (int i = 0; i < lista.getSize(); i++) {
            if (a == iterator.element) A = iterator;
            //if (b == iterator.element) B = iterator;
            if (c == iterator.element) C = iterator;
            //if (d == iterator.element) D = iterator;
            iterator = iterator.succ;
        }

        while (C != null) {
            lista.insertBefore(C.element, A);
            lista.delete(C);
            C = C.succ;
        }
