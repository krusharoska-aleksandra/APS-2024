...
//
        ...
        int maxDamage = 0;
        SLLNode<Card> maxTmp = firstFriendCards.getFirst();

        while (first != null) {
            if (first.element.damage() > maxDamage) {
                maxDamage = first.element.damage();
                maxTmp = first;
            }
            first = first.succ;
        }
        ...
        secondFriendCards.insertBefore(maxTmp.element, second);
        firstFriendCards.delete(maxTmp);
        //secondFriendCards.insertBefore(firstFriendCards.delete(maxTmp), second);
    }
